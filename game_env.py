...
def pick_value(rider, track, paces):
    ...
    for card in st.session_state.cards[rider]['cards'][0:4]:
        sv = get_slipstream_value(st.session_state.cards[rider]['position'],st.session_state.cards[rider]['position'] + card[1], track)
        if sv < 3:
...
def get_random_track():
    ...
    for i in range(0, 55 + random.randint(0, 6) + random.randint(0, 6)):
        ...
        if track[i] == '_' and random.random() > 0.4:
            track = track + '_'

        elif track[i] != '_' and random.random() > (0.3 - int(track[i]) * 0.05):
...
col4.caption('length = 60=170km + 1 pr 12')
    ...
    with col4:
        ...
        if sv > 2:
            ...
            if not chosen_card:
                ...
                chosen_card, managed, new_minus = choose_card_to_play(rider['cards'][0:6], sv, penalty, speed, chosen_value, True)
        ...
        if sv <= 2:
            ...
            if not chosen_card:
                chosen_card, managed, new_minus = choose_card_to_play(rider['cards'][0:4], sv, penalty, speed, chosen_value, True)
                if track[rider['position']] == '_':
